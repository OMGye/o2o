...
function getAllbill(pageNum) {
	// body...
	var url='/o2o/admin/proposeInfo/list.do?pageNum='+pageNum+'&pageSize='+20;
	$.ajax({
			...
			success : function(v) {
				if(v.status==1){
					...
					for (var i =  0; i < v.data.list.length; i++) {
						...
						$("#OrderList").append(
							'<tr><td>'+val.proposeId+'</td><td>'+val.userId+'</td><td>'+val.userName+'</td><td>'+changeTime(val.createTime)+'</td><td><a onclick="show('+val.proposeId+')" class="btn btn-warning">内容</a></td></tr><tr style="display:none" id="show'+val.proposeId+'"><td colspan="5">'+val.proposeContent+'</td></tr>'
						);
